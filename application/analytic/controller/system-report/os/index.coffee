_config = ($stateProvider) ->
  $stateProvider.state "analytic.system-report.os",
    url : "/os"
    templateUrl : '/template/analytic/controller/system-report/os/view.html'
    controller : 'analytic.controller.system-report.os.Ctrl'
...
_controller = ($rootScope, $scope, $http, ApiService, UtitService, $state, $timeout, $location, $interval,
  ...
  $rootScope.metaPageTitle = 'OS Report'
  $scope.param =
    startDate : ''
    endDate : ''
...
#    type : 'percent'
  ...
  $scope.actionDownload = ()->
    ...
    XLSX.utils.book_append_sheet(wb, ws, "os_report")
    #write workbook and force a download
    XLSX.writeFile(wb, "#{moment().format('DD/MM/YYYY')}_os_report.xlsx")
  ...
  $scope.loadData = ()->
    ...
    $.blockUI()
    AnalyticService.systemReport.osTable params, (err, result)->
      $.unblockUI()
    ...
    AnalyticService.systemReport.osChart params, (err, result)->
      return if err or !result
      $scope.allDataChart = result.data
      $scope.parseDataChart("Duration", 'total_duration')

    AnalyticService.systemReport.osSummary params, (err, result)->
      return if err or !result
...
window.app
  .config _config
  .run _run
  .controller 'analytic.controller.system-report.os.Ctrl', _controller
